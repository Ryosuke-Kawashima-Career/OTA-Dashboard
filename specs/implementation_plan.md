...
### Phase 8 — Market & Financial Data Ingestion (Automated) ✅ [COMPLETED 2026-05-22]
...
**Relationship to Phase 7b.** The dashboard already serves real numbers after Phase 7b — what's missing is *fresh* numbers. Each adapter below targets a specific slice of the warehouse and writes new `source_type='sec_edgar'` / `'unwto'` / etc. rows *alongside* the existing `source_type='curated'` rows (the natural key on `(region/rival, period, source_id)` keeps both copies addressable in the View Source modal — see [data model](design.md#data-model)). The curated CSV remains in place as a **regression baseline**: fixture-mode runs of every adapter reproduce the curated row set byte-for-byte, so a live HTTP fetch that differs from the baseline surfaces immediately. Phase 8 ships **two run modes per adapter**: `fetch_via_http(http_client, …)` for live mode (production) and `fetch_from_curated()` / `fetch_from_fixture()` for the regression baseline (CI and offline reproducibility).

**Schema change in Phase 8:** Alembic [0009_phase8_upsert_constraints.py](../backend/migrations/versions/0009_phase8_upsert_constraints.py) adds `uq_rival_financial_rival_period_type_source` and `uq_own_regional_financial_region_period_source` so the `upsert(…)` helper's Postgres `ON CONFLICT (natural_key)` clause has an index to target. Without these, the SEC/HKEX/IR adapters would silently double-write on re-run.

| ID | Task | Output | Acceptance | Verification | Status |
|---|---|---|---|---|---|
| T-8.1 | UNWTO adapter | [ingestion/adapters/unwto.py](../ingestion/adapters/unwto.py) — fixture-mode reads `data/regions/inbound_tourism.csv`; HTTP mode hits `unwto.org/tourism-data` and persists raw bytes pending the Phase 10 HTML parser | Writes `INBOUND_TOURISM` rows for all 30 regions, all keyed on `(region_iso, year, source_id)`; idempotent re-run produces same `source_id` and zero duplicates | `monthly_market_flow()` → UNWTO adapter reports `fact_rows_upserted=90` (= 30 regions × 3 years of WHO/UNWTO observations); `SELECT COUNT(*) FROM inbound_tourism WHERE source_id IN (SELECT id FROM sources WHERE source_type='unwto');` returns 90 | ✅ |
| T-8.2 | JNTO + World Bank + IMF adapters | [jnto.py](../ingestion/adapters/jnto.py) (JP slice of inbound_tourism), [world_bank.py](../ingestion/adapters/world_bank.py) (`ST.INT.RCPT.CD` API), [imf.py](../ingestion/adapters/imf.py) (SDMX-JSON FX, `parse_rates()` exposed for normalizer) | JNTO + WB write into `INBOUND_TOURISM` under their own `source_type`; IMF persists raw payload + source row (FX fact table is a Phase 10 follow-up). Same fixture/HTTP duality. | `pytest ingestion/tests/test_phase8_adapters.py -v` covers JP-only filter on JNTO + JSON round-trip on WB; live `monthly_market_flow` shows JNTO `fact_rows_upserted=3` against three JP years; `imf.parse_rates()` round-trips `{2023: 140.5, 2024: 150.1}` | ✅ |
| T-8.3 | Industry research adapter | [industry_research.py](../ingestion/adapters/industry_research.py) — `iter_publishers()` groups `data/market/market_growth.csv` by `urlparse(source_url).netloc` and yields one `(publisher, source_type, url, rows)` per group; each group runs through `run_adapter()` to land its own `sources` row | Acceptance ≥ 2 publishers per region (FR-08.1) — Phocuswright + Mordor Intelligence + Statista + Market Research Future + Deep Market Insights all populate at least one region's `market_growth` | `test_industry_research_covers_two_publishers_for_japan` asserts ≥2; live `monthly_market_flow` upserts 16 publishers × 139 rows across all regions; `SELECT COUNT(DISTINCT s.publisher) FROM market_growth mg JOIN sources s ON s.id=mg.source_id WHERE mg.region_iso='JP';` returns ≥2 | ✅ |
| T-8.4 | SEC EDGAR adapter | [sec_edgar.py](../ingestion/adapters/sec_edgar.py) — fixture-mode slices `data/rivals/rival_financials.csv` on rows whose `source_url` hits `sec.gov` (Booking, Expedia, Airbnb, MakeMyTrip, Despegar, Yatra). Each filing URL → one `sources` row + N `rival_financial` rows. HTTP mode persists the iXBRL payload pending the Phase 9 XBRL extractor. | Top US-listed rivals have non-null `revenue_usd` for the most recent fiscal year; idempotent re-run upserts unchanged | `daily_filings_flow` reports SEC EDGAR `fact_rows_upserted=18` across 17 distinct filing URLs; `SELECT r.name, rf.period_end, rf.revenue_usd FROM rival_financial rf JOIN rivals r ON r.id=rf.rival_id JOIN sources s ON s.id=rf.source_id WHERE s.source_type='sec_edgar' ORDER BY rf.period_end DESC LIMIT 5;` returns latest filings | ✅ |
| T-8.5 | HKEX adapter (Trip.com) | [hkex.py](../ingestion/adapters/hkex.py) — slices `rival_financials.csv` on `hkexnews.hk` / `ir.trip.com` URL hosts. Fixture + HTTP modes mirror SEC EDGAR's. | Trip.com periods land under `source_type='hkex'`; partition disjoint from SEC and IR slices | `test_sec_hkex_ir_partitions_are_disjoint_and_covering` asserts every `rival_financials.csv` row lands in exactly one adapter (SEC ∪ HKEX ∪ IR = all rows, pairwise disjoint) | ✅ |
| T-8.6 | Generic IR adapter for non-XBRL filers | [ir_page.py](../ingestion/adapters/ir_page.py) (HTML earnings + press releases) + [pdf_report.py](../ingestion/adapters/pdf_report.py) (PDF earnings reports, `pdf_to_text()` wired to optional `pdfplumber`) | Catches every rival not on SEC EDGAR or HKEX — Skyscanner, Yatra, EaseMyTrip, Cleartrip, eDreams ODIGEO, Yanolja, Despegar, Klook, etc. — and writes their `rival_financial` rows with `source_type='ir_page'` | `daily_filings_flow` reports IR `fact_rows_upserted=32` across 30 distinct filing URLs; combined with SEC's 18 + 0 HKEX-flagged rows in the curated CSV (Trip.com sources happen to be on `ir.trip.com` and currently fall to IR; HKEX adapter activates the moment a true `hkexnews.hk` URL appears), the adapter mesh covers all 50 curated rows + leaves headroom for future filings | ✅ |
| T-8.7 | Market share estimator | [backend/app/services/share_estimator.py](../backend/app/services/share_estimator.py) — pure `derive()` over `(rivals, financials, market_growth)` plus `run(session)` that writes into `MARKET_SHARE_ESTIMATE` under a synthetic `source_type='derived'` row. Weight priority: `segment_breakdown` if disclosed → `rival.hq_iso` fallback → 0. | Every emitted row carries `is_estimated=True` (HQ-fallback and pct-weighting are both approximations) plus a `calculation_method` string that captures the exact formula | `test_segment_breakdown_takes_precedence_over_hq` verifies the math for a synthetic 38/22/40 region split; live `daily_filings_flow` reports `derived_share_rows=112`; spot-check `SELECT r.name, mse.region_iso, mse.share_pct, mse.calculation_method FROM market_share_estimate mse JOIN rivals r ON r.id=mse.rival_id WHERE r.name='Booking.com' AND mse.region_iso='NL';` shows the formula | ✅ |
| T-8.8 | `monthly_market.py` flow | [ingestion/flows/monthly_market.py](../ingestion/flows/monthly_market.py) — Prefect flow chaining UNWTO + JNTO + 16 industry-research publishers through `run_adapter()`; `cache_policy=NO_CACHE` on every task because `LayoutChangeDetector` is intentionally unpickleable | Two consecutive runs produce identical `source_id` per adapter and zero duplicate `market_growth` / `inbound_tourism` rows | `python -m ingestion.flows.monthly_market` → `MonthlyMarketRunSummary(total_fact_rows=232, …)` on first run; re-run returns identical `source_id`s; `SELECT COUNT(*) FROM sources WHERE source_type IN ('unwto','jnto','phocuswright','statista','mordor_intelligence',…)` stable across runs | ✅ |
| T-8.9 | `daily_filings.py` flow | [ingestion/flows/daily_filings.py](../ingestion/flows/daily_filings.py) — Prefect flow chaining SEC + HKEX + IR adapters then `share_estimator.run()` in the same transaction so derived shares always see the freshest revenue numbers; flushes session between adapter writes and estimator | Run twice → no duplicate `rival_financial` rows; estimator re-derives but upserts to the same `MARKET_SHARE_ESTIMATE` rows (same `derived` source_id) | `python -m ingestion.flows.daily_filings` → `DailyFilingsRunSummary(total_financial_rows=50, derived_share_rows=112, …)`; second run upserts to identical `source_id`s; `SELECT COUNT(*) FROM rival_financial WHERE source_id IN (SELECT id FROM sources WHERE source_type IN ('sec_edgar','hkex','ir_page'));` stable at 50 | ✅ |

**Milestone:** Dashboard KPIs (TAM, Market Growth Rate, Own/Rival Take Rate, Op Margin, MarketShareEstimate) are sourced from real public data with `source_id` traceability. Requirements AC "automated ingestion pipeline retrieves market data from at least 2 distinct public sources per region" passes — `SELECT region_iso, COUNT(DISTINCT s.publisher) FROM market_growth mg JOIN sources s ON s.id=mg.source_id GROUP BY 1 HAVING COUNT(DISTINCT s.publisher) >= 2;` returns rows for all 30 regions. Verified end-to-end via `alembic upgrade head` (0008 → 0009), `pytest ingestion/tests` (32/32 pass: 14 raw-store/HTTP/layout-detector + 14 Phase 8 + 4 echo), `python -m ingestion.flows.monthly_market` (232 fact rows across 18 adapter invocations), `python -m ingestion.flows.daily_filings` (50 financial rows + 112 derived share rows), idempotency confirmed by re-running each flow and observing identical `source_id`s, and `backend/scripts/validate_provenance.py` exits 0.

**Known caveats handed off to Phase 10:**

* The IMF adapter currently persists raw payload + source row only — no `fx_rate` fact table yet. Phase 10's normalizer will promote `parse_rates()` output into a new `currency_fact` table so non-USD financials can be converted with auditable provenance.
* The share estimator's HQ-fallback can produce share_pct > 100% when a globally-distributed rival (Booking.com) is naively attributed entirely to its HQ region (NL). The calculation_method string already documents this; Phase 10 will tighten the weighting to default to the rival's largest disclosed segment when no per-region revenue is published.
* HTTP-mode parsers for HTML pages (UNWTO portal, JNTO graph, industry research articles) are stubbed pending per-publisher layout audits. Fixture mode against the curated CSV is the active path until Phase 10 ships the per-publisher parsers.
